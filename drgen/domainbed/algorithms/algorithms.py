...
from domainbed.lib.misc import random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts
...
class ERM(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        loss = weighted_loss(F.cross_entropy(self.predict(all_x), all_y))
...
class AbstractMMD(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        for i in range(nmb):
            objective += weighted_loss(F.cross_entropy(classifs[i], targets[i]))
...
class SagNet(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer_f = torch.optim.SGD(self.network_f.parameters(),lr=hparams["lr"],momentum=0.9,weight_decay=hparams["weight_decay"])
        self.optimizer_c = torch.optim.SGD(self.network_c.parameters(),lr=hparams["lr"],momentum=0.9,weight_decay=hparams["weight_decay"])
        self.optimizer_s = torch.optim.SGD(self.network_f.parameters(),lr=hparams["lr"],momentum=0.9,weight_decay=hparams["weight_decay"])
...
class Fishr(Algorithm):
    ...
    def _init_optimizer(self):
        print(f'the learning rate is {self.hparams["lr"]}')
    ...
    def update(self,x, y, **kwargs):
        ...
        all_x = torch.cat([xi for xi in x])
        all_y = torch.cat([yi for yi in y])
        len_minibatches = [xi.shape[0] for xi in x]
        print(f'length of minibatches is {len_minibatches}')
        print(f'the learning rate is {self.hparams["lr"]}')
