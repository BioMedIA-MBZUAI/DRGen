...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    for (env, _), batch_size in iterator.train(zip(in_splits, batch_sizes)):
        print(str(len(env))+str(batch_size)+" the length, batch size of the environment named "+str(env))
    steps_per_epochs = [
        len(env) / batch_size
        for (env, _), batch_size in iterator.train(zip(in_splits, batch_sizes))
    ]
    ...
    scheduler = get_scheduler(
        hparams["scheduler"],algorithm.optimizer,hparams["lr"],n_steps)
    ...
    for step in range(n_steps):
        ...
        # print(f'the learning rate is {hparams["lr"]}')
